...
app.post("/login", async(req: Server[0], res: Server[1], next: Server[2]) => {
    ...
    if (user && sha3_256(user.salt + password) === user.password) return res.send({ 
        success: true, 
        token: jwt.sign({ username }, SECRETHASH, { expiresIn: '2h' }), 
        status: 200, msg: "로그인 되었습니다."
    }).status(200)
    else res.send({ 
        success: false, 
        msg: "비밀번호가 틀렸거나 유저가 없습니다."
    }); 
